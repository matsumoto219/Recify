...
以下の共通テストアカウントには、架空のサンプルレシートを複数登録しています。
レシート一覧、検索、詳細表示、明細や集計結果などをご確認いただけます。

- メールアドレス: `recify-sample-acc@protonmail.com`
- パスワード: `password123@`

### 自分でデータを登録して試す

ログイン画面で「ゲストとして試す」を選択すると、ログイン情報を入力せずに機能を体験できます。

ゲストアカウントはレシート0件の状態で作成されます。ご自身でデータを登録しながら、作成・編集などの操作をご確認ください。

ゲストアカウントとそのデータは一時的な体験用で、一定期間利用がない場合に削除されます。
個人情報を含むデータは登録しないでください。
