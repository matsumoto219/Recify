module SystemOperations
  class SystemSettingUpdateExecutor
    ...
    def initialize(key:, value:, actor:, reason:, request:, reauthentication:, confirmation: nil)
      @key = key.to_s.strip
      @raw_value = value
      @actor = actor
    ...
    attr_reader :key, :raw_value, :actor, :reason, :request, :reauthentication, :confirmation
    ...
    def update_setting_record!(casted_value)
      setting = SystemSetting.find_or_initialize_by(key: key)
      setting.assign_attributes(
        value: SystemSettings.stored_value(SystemSettings.audit_value(casted_value)),
