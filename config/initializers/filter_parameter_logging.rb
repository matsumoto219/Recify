...
security_exact_key_filter = /\A(?:credential|raw[_-]?id|attestation[_-]?object|client[_-]?data[_-]?json|authenticator[_-]?data|public[_-]?key|credential[_-]?id|challenge|signature|user[_-]?handle|totp|otp[_-]?attempt|totp[_-]?code|totp[_-]?secret|encrypted[_-]?totp[_-]?secret|recovery[_-]?codes?|backup[_-]?codes?|provisioning[_-]?uri|otpauth|code[_-]?digest|session[_-]?uid(?:[_-]?digest)?|raw[_-]?response|two[_-]?factor|second[_-]?factor|one[_-]?time[_-]?password|authorization|endpoint|signed[_-]?id|blob[_-]?key|storage[_-]?key|checksum|content[_-]?md5)\z/i
