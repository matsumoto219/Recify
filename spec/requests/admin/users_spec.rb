...
RSpec.describe 'Admin users', type: :request do
  ...
  def stub_fresh_admin_reauthentication
    ...
    allow_any_instance_of(Admin::UsersController).to receive(:admin_reauthentication_context) do |controller|
  ...
  describe 'GET /admin/users' do
    ...
    it 'adminユーザーはindexを閲覧でき、navigationにユーザー管理が出る' do
      ...
      aggregate_failures do
        ...
        expect(email_node.text).to eq('target-user@example.com')
        expect(email_node['class']).to include('inline-flex')
        expect(email_node['class']).not_to include('grid-cols')
        expect(email_node.css('span').map(&:text)).to eq([ 'target-user', '@', 'example.com' ])
  ...
  describe 'GET /admin/users/:id' do
    it 'adminユーザーはshowを閲覧でき、security/passkey/receipt/audit状態を確認できる' do
      ...
      aggregate_failures do
        ...
        expect(email_display_node.text).to eq('show-user@example.com')
