...
RSpec.describe 'Mailer layout', type: :mailer do
  ...
  def expect_soft_mailer_palette(html)
    aggregate_failures do
      ...
      expect(html).to include('.mailer-brand { padding: 0 0 14px 10px !important; }')
      expect(html).to include('class="mailer-brand" align="left" style="padding: 0 0 16px 14px;"')
      ...
      expect(html).not_to include('height="48"')
      expect(html).not_to include('margin: -6px')
      expect(html).not_to include('margin-left: -14px')
